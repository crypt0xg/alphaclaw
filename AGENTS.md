...
4. When ready for production, publish a stable release version (for example `0.3.2`):
   ...
   - Pin all deployment templates on `main` to that release: update `~/Projects/openclaw-railway-template`, `~/Projects/openclaw-render-template`, and `~/Projects/openclaw-apex-template` so `@chrysb/alphaclaw` and `openclaw` match AlphaClaw’s `package.json` for the release, run `npm install` in each repo, commit `package.json` and `package-lock.json`, and push. Skipping any template leaves it on `latest` and causes drift between platforms.
